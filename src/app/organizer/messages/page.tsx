        ...
        {conversations.length > 0 ? (
          conversations.map((conv) => (
            <Link key={conv.user.id} href={`/organizer/messages/${conv.user.id}`}>
              <Card className="group border-border/60 bg-foreground/2 hover:border-primary/30 hover:bg-primary/5 transition-all duration-300 rounded-4xl overflow-hidden">
                <CardContent className="p-8 flex items-center justify-between gap-6">
                  <div className="flex items-center gap-6">
                    <div className="h-16 w-16 rounded-2xl bg-foreground/3 border border-border/60 flex items-center justify-center relative">
                      ...
                      {conv.unreadCount > 0 && (
                        <span className="absolute -top-2 -right-2 h-6 w-6 rounded-full bg-primary text-[10px] font-black text-primary-foreground flex items-center justify-center">
                          {conv.unreadCount}
                        </span>
                      )}
                    </div>
          ...
          <Card className="py-24 text-center space-y-6 bg-foreground/2 border-dashed border-border rounded-[2.5rem]">
            <div className="h-24 w-24 rounded-3xl bg-foreground/3 flex items-center justify-center mx-auto border border-border text-foreground/10">
              <MessageSquare className="h-12 w-12" />
            </div>
            <div className="space-y-2">
              <h3 className="text-2xl font-black tracking-tight uppercase">No messages yet</h3>
              <p className="text-muted-foreground max-w-md mx-auto">You haven't received any messages from users yet.</p>
            </div>
          </Card>
        )}
      </div>
